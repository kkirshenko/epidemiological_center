...
Информационная система санитарно-эпидемиологического центра автоматизирует полный цикл надзорной деятельности: ведение реестра поднадзорных организаций, планирование и проведение выездных проверок, фиксацию нарушений с контролем сроков устранения, формирование отчётности. Система ориентирована на инспекторов, лаборантов и администраторов центров гигиены и эпидемиологии. Решение построено на стеке Java 17 + Spring Boot 3.2.x + PostgreSQL 15 с контейнеризацией через Docker.

### 1.2. Цели архитектуры

| Цель | Описание |
|------|----------|
| **Разделение ответственности** | Чёткое разделение на контроллеры, сервисы, репозитории и сущности для снижения связности |
| **Тестируемость** | Каждый слой тестируется изолированно через JUnit 5 + Mockito; покрытие кода ≥ 41% |
| **Масштабируемость** | Модульная архитектура позволяет добавлять новые функции без переписывания ядра |
| **Поддерживаемость** | Стандартные паттерны Spring, документирование через OpenAPI, контейнеризация |
| **Безопасность** | JWT-аутентификация, ролевая модель, валидация входных данных, защита от веб-угроз |
...
| **Администраторы системы** | Простота развёртывания, мониторинг работоспособности, управление пользователями и правами |
...
| **Заказчик (учебный)** | Работоспособность системы, покрытие тестами, документирование архитектуры по стандарту arc42 |
...
|-------------|----------|
| **Язык серверной части** | Java 17+ (LTS) |
...
| **Тестирование** | JUnit 5, Mockito, JaCoCo 0.8.12 |

### 2.2. Бизнес-ограничения

| Ограничение | Значение |
|-------------|----------|
| **Бюджет** | 0 руб. (учебный проект, открытый исходный код) |
| **Сроки** | 1 семестр (~18 недель) |
| **Команда** | 1 разработчик (фулстек) |
| **Лицензирование** | Только открытые технологии (MIT/Apache 2.0) |
| **Соответствие регуляторам** | ФЗ-152 «О персональных данных», приказы ФСТЭК, СанПиН |
| **Язык интерфейса** | Русский (для пользователей), английский (для кода и документации) |
...
Описание архитектуры PCMEF по уровням

#### Presentation (P) — Уровень представления

**Назначение:** Отображение пользовательского интерфейса и взаимодействие с пользователем.

**Компоненты:**
- `MainFrame` — главное окно приложения
- `InspectionDialog` — диалоговое окно для работы с проверками
- `ViolationsPanel` — панель отображения нарушений
- `RegistryTable` — таблица реестра организаций
- `ReportPreviewWindow` — окно предварительного просмотра отчётов

**Ответственность:** 
- Приём ввода данных от пользователя
- Валидация форм на уровне UI
- Отображение результатов обработки данных
- Условный рендеринг элементов в зависимости от роли пользователя

**Зависимости:** Использует уровень Control (C) через паттерн MVC.

---

#### Control (C) — Уровень управления

**Назначение:** Обработка пользовательских запросов, координация между Presentation и Mediator.

**Компоненты:**
- `InspectionFormController` — управление формами проверок
- `ViolationFormController` — управление формами нарушений
- `RegistryController` — управление реестром организаций
- `ReportActionController` — генерация и экспорт отчётов

**Ответственность:**
- Приём HTTP-запросов (для Web) или событий UI (для Desktop)
- Валидация входных данных через `@Valid`
- Маппинг DTO ↔ Entity
- Возврат ответов с соответствующими HTTP-статусами
- Обработка исключений уровня контроллера

**Зависимости:** Делегирует бизнес-операции уровню Mediator (M).

---

#### Mediator (M) — Уровень бизнес-логики

**Назначение:** Оркестрация бизнес-процессов, применение правил предметной области.

**Компоненты:**
- `InspectionMediator` — управление жизненным циклом проверок
- `ViolationMediator` — учёт нарушений и контроль сроков
- `ReportMediator` — формирование отчётности (акты, протоколы)

**Ответственность:**
- Оркестрация процессов и бизнес-логика
- Валидация бизнес-правил (например, расчёт категорий риска)
- Управление транзакциями через `@Transactional`
- Координация между несколькими сущностями
- Применение стратегий и паттернов проектирования

**Зависимости:** 
- Manipulates (манипулирует) сущностями уровня Entity (E)
- Uses services (использует сервисы) уровня Foundation (F)

---

#### Entity (E) — Уровень сущностей

**Назначение:** Модель предметной области, отображаемая на таблицы базы данных.

**Компоненты:**
- `Organization` — поднадзорная организация
- `Inspection` — запись о проверке
- `Violation` — выявленное нарушение
- `Status` — справочник статусов
- `ReportType` — типы отчётных документов

**Ответственность:**
- Хранение данных предметной области
- Аннотации JPA (`@Entity`, `@Column`, `@OneToMany`)
- Бизнес-методы сущностей (например, `isOverdue()`)
- Связи между агрегатами через внешние ключи

**Зависимости:** Не зависит от других уровней (чистая доменная модель).

---

#### Foundation (F) — Инфраструктурный уровень

**Назначение:** Абстракция доступа к данным и вспомогательные сервисы.

**Компоненты:**

**db (доступ к данным):**
- `AbstractRepository` — базовый репозиторий с общими методами
- `EntityManagerFactoryUtil` — утилита создания EntityManager

**repository (конкретные репозитории):**
- `InspectionRepo` — доступ к данным проверок
- `ViolationRepo` — доступ к данным нарушений
- `OrgRepo` — доступ к данным организаций

**report (генерация отчётов):**
- `PdfGenerator` — генерация PDF-документов
- `TplLoader` — загрузка шаблонов отчётов

**util (вспомогательные утилиты):**
- `DateFormatter` — форматирование дат
- `ValidationUtil` — общие методы валидации

**Ответственность:**
- Абстракция CRUD-операций через Spring Data JPA
- Кастомные запросы через `@Query`
- Оптимизация запросов (JOIN FETCH для решения N+1)
- Генерация документов по шаблонам
- Вспомогательные функции без бизнес-логики

**Зависимости:** Не зависит от верхних уровней (стабильный фундамент).
